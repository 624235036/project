...
if (isset($_GET['scoreclass'])) {
  $id = $_GET['id_class'];
  $id_s = $_GET['id_school'];
  $sqlQuery = $conn->query("SELECT `name_header`, scoreclass.`id_class`, `id_school`, class.name_class, AVG(`avgscore`) as avgscore 
                            FROM `scoreclass`
                            INNER JOIN class ON class.id_class = scoreclass.`id_class`
                            WHERE scoreclass.id_class = $id AND id_school = $id_s
                            GROUP BY `name_header`,scoreclass.`id_class`,`id_school`, class.name_class");
  $sqlQuery->execute();
  ...
  <div class="container">
    <div class="row">
      ...
      <h3 align="center">รายงานภาพรวมสมรรถนะ 5 ด้าน</h3>
        <h5 align="center">(ชั้นปี <?php if(!$result) {echo'ไม่พบข้อมูล';}elseif($rs) {echo $rs['name_class']; }?>)</h5>
        <!--devbanban.com-->
