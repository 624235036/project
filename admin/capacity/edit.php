...
if (isset($_POST['btn_update'])) {
    ...
    $sql = $conn->prepare("UPDATE tb_question SET question = :question WHERE id_question = id_question");
    $sql->bindParam(':id_question', $id_question);
...
<body>
    ...
    <form action="edit.php" method="post" class="form-horizontal mt-5">
        ...
        if (isset($_GET['id_question'])) {
            ...
            $select_stmt = $conn->query("SELECT * FROM tb_question WHERE id_question = $id_question");
            $select_stmt->execute();
            $data = $select_stmt->fetch();
        }
        ?>
        <div class="form-group">
            <label for="id_question">ลำดับที่:</label>
            <input type="text" readonly value="<?= $data['id_question']; ?>" class="form-control" name="id_question">
        </div>
        <div class="form-group">
